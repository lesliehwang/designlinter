...
figma.ui.onmessage = msg => {
  if (msg.type === "close") {
    ...
    // ADDED TO PREVENT TEAM LOGOS FROM BEING LINTED
  ...
  // ADDED THIS FUNCTION TO PREVENT TEAM LOGOS FROM BEING LINTED
  function findName(node) {
    let logoName = node.name;
    let lastCharacter = logoName.substring(
      logoName.length - 1,
      logoName.length
    );
    if (lastCharacter == ")") {
      node.locked = true; //lock so that it wont get linted

      //avoid repeats: go through current array, only add node if the ID is different
      let flag = false;
      for (let i = 0; i < lockedLogo.length; i++) {
